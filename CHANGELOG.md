...
- **Agent browser control:** agents can now open a page in the browser panel and work with it — read what is on screen, click, type, scroll, look at how an element renders, switch between mobile, tablet and desktop layouts, and save a screenshot into the project so a change can be shown rather than described — so they can check their own work instead of describing what they expect. Available as a separate OpenChamber Web tool that can be turned off on its own.
...
- Browser: the panel can hold several pages at once — the plus button opens another tab — and a tab now shows the page's own icon on the desktop app.
- Browser: the address bar now suggests pages already visited in this project, matching on part of an address or a page title; an entry can be removed from the list.
...
- Browser: when OpenChamber runs on another machine, the desktop app opens its dev servers through a local port, so pages load with working hot reload and developer tools. Links and redirects to another local port — a docs server or an API behind a dev gateway — stay on that machine instead of being looked for on yours. In a web browser tab, only dev servers on your own machine can be opened.
