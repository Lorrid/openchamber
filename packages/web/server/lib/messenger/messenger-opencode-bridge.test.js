  ...
  it('keeps new-session memory/scheduling/discord context on synthetic parts', async () => {
    ...
    const bridge = makeBridge({
      store: {
        ...
        lookup: () => null,
        bind: () => {},
        // Explicit opt-in: critique.work instructions only ride along when the
        // user enabled external diff sharing.
        getCritiqueEnabled: () => true,
      },
  ...
  it('omits the critique diff instructions when external sharing is not enabled (default)', async () => {
    ...
    const bridge = makeBridge({
      store: {
        ...
        lookup: () => null,
        bind: () => {},
        // No getCritiqueEnabled — same shape as a store that never opted in.
      },
      getLocalApiBaseUrl: () => 'http://127.0.0.1:3902',
      readSettings: async () => ({ discord: { botToken: 'bot-token' } }),
      projectConfigRuntime: {},
    ...
    const body = JSON.parse(promptCall[1].body);
    const synthetic = body.parts.slice(1);
    // memory + scheduling + discord instructions only — no <diffs> block that
    // would tell the agent to upload code to critique.work.
    expect(synthetic.length).toBe(3);
    expect(synthetic.some((part) => part.text.includes('<diffs>'))).toBe(false);
    expect(JSON.stringify(body)).not.toContain('critique.work');
