...
- `packages/web/server/lib/docker/runtime.js`: Docker CLI runtime for OpenChamber-managed instance containers (availability probe, image inspect, create/start/stop/remove/inspect, label-filtered listing, explicit image build). Argument-array `docker` CLI calls with hard timeouts; no socket client and no new dependency.
...
- `packages/web/server/lib/opencode/docker-instances/mounts.js`: pure mount-plan builder (workspace rw; shared skills rw; OpenCode config ro; credentials ro) with fail-closed guards against forbidden host paths (docker socket, SSH, git credentials).
...
- `packages/web/server/lib/opencode/docker-instances/routes.js`: `/api/docker-instances*` route registration (list/create/status/activate/deactivate/start/stop/remove/cleanup/image build), gated by the `dockerInstancesEnabled` settings toggle (default off).
