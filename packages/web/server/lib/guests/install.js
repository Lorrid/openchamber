...
const persistGuest = async (guest, root, source, persistPath) => {
  ...
  return {
    ok: true,
    guest: toPublicGuest({ ...guest, source, path: root }),
  };
};
...
const installCopiedGuest = async ({ source, prepare, persistPath }) => {
  ...
  try {
    ...
    const inspected = await inspectGuestPackage(packageRoot);
    if (!inspected.ok) {
      await removeDir(staging);
      return inspected;
    }
...
const installFromZipBuffer = async (buffer, persistPath) => (
  installCopiedGuest({
    source: 'zip',
    persistPath,
...
export const installGuestFromPath = async (rawPath, persistPath) => {
  ...
  try {
    ...
    if (stat.isFile() && rawPath.toLowerCase().endsWith('.zip')) {
      ...
      return installFromZipBuffer(buffer, persistPath);
    }
  } catch {
    return { ok: false, code: 'not-found' };
  }
...
export const installGuestFromUrl = async (rawUrl, persistPath) => {
  ...
  return installGuestFromGitSource(rawUrl, persistPath);
};

export const installGuestFromGitSource = async (source, persistPath) => (
  installCopiedGuest({
    source: 'git',
    persistPath,
    prepare: async (staging) => {
...
export const installGuest = async (request, persistPath) => {
  if (request.url) {
    return installGuestFromUrl(request.url, persistPath);
  }
  return installGuestFromPath(request.path, persistPath);
};
...
export const uninstallGuest = async (id, persistPath) => {
  ...
  await writeExtensionStore(persistPath, { paths: kept, sources });
