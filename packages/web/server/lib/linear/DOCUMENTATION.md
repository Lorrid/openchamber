...
This module owns Linear OAuth, issue lookup, Linear-team-to-project mapping, and session status comments on Linear issues. Credentials live on the OpenChamber server, so web, desktop, and a phone paired to that host share them. You can store more than one Linear workspace; exactly one is current. Issue list, mapping, and new OAuth default to the current workspace. Session status comments use the workspace that started the session. The chat picker lists issues and attaches them to a message. New Worktree can also start from a Linear issue in the currently active project. A session started from a Linear issue posts started/completed/failure comments, each with an OpenChamber session link.
...
- `GET /api/linear/issues/list?query=&cursor=`: open issues from the current workspace, or a search. An identifier or Linear URL returns that issue even if it is completed. Never returns tokens.
