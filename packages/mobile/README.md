...
- The phone tab shell owns an arbitrary-depth metadata-only chat route stack and renders a two-page DOM window containing the top route plus its immediate predecessor. Root-to-chat and parent-to-child pushes move the incoming page from 100% to 0 while the predecessor shifts to -8%. Interactive back therefore reveals the real parent, commits session selection after the outgoing page reaches 100%, and supports grandchild-to-root traversal one level at a time. A deep-linked child gains its immediate predecessor when the authoritative parent entity resolves.
...
- Flow-mobile Settings and Scheduled Tasks keep one persistent, viewport-bounded root layer mounted behind their independently scrolling detail surface. Root routes register both layers explicitly; gesture code never discovers an underlay by querying the outgoing page's ancestors. Only those bounded layers move, long tab documents never become compositor layers, and horizontal overflow remains clipped throughout the interaction.
