package com.openchamber.app;

import com.getcapacitor.Plugin;
import com.getcapacitor.PluginCall;
import com.getcapacitor.PluginMethod;
import com.getcapacitor.JSObject;
import com.getcapacitor.annotation.CapacitorPlugin;
import org.json.JSONArray;

@CapacitorPlugin(name = "OpenChamberShare")
public class OpenChamberSharePlugin extends Plugin {
    @PluginMethod public void updateCatalog(PluginCall call) { try { JSONArray entries = call.getArray("entries"); if (entries == null) throw new IllegalArgumentException("entries is required"); OpenChamberShareStore.updateCatalog(getContext(), entries); call.resolve(); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void listPending(PluginCall call) { try { JSObject result = new JSObject(); result.put("envelopes", OpenChamberShareStore.pending(getContext())); call.resolve(result); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void listDrafts(PluginCall call) { try { JSObject result = new JSObject(); result.put("drafts", OpenChamberShareStore.pendingDrafts(getContext())); call.resolve(result); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void pendingAssistantOpen(PluginCall call) { try { JSObject result = new JSObject(); result.put("request", OpenChamberShareStore.pendingAssistantOpen(getContext())); call.resolve(result); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void ackAssistantOpen(PluginCall call) { String id = call.getString("requestID"); if (id == null || !id.matches("[A-Za-z0-9-]{1,80}")) { call.reject("A valid requestID is required."); return; } try { OpenChamberShareStore.acknowledgeAssistantOpen(getContext(), id); call.resolve(); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void ack(PluginCall call) { String id = call.getString("operationID"); if (id == null || !id.matches("[A-Za-z0-9-]{1,80}")) { call.reject("A valid operationID is required."); return; } try { OpenChamberShareStore.acknowledge(getContext(), id); call.resolve(); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void releaseFiles(PluginCall call) { String id = call.getString("operationID"); if (id == null || !id.matches("[A-Za-z0-9-]{1,80}")) { call.reject("A valid operationID is required."); return; } try { OpenChamberShareStore.releaseAcknowledged(getContext(), id); call.resolve(); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    @PluginMethod public void cancelDraft(PluginCall call) { String id = call.getString("draftID"); if (id == null || !id.matches("[A-Za-z0-9-]{1,80}")) { call.reject("A valid draftID is required."); return; } try { OpenChamberShareStore.cancelDraft(getContext(), id); call.resolve(); } catch (Exception error) { call.reject(error.getMessage(), error); } }
    void emitReceived(String operationID) { JSObject event = new JSObject(); event.put("operationID", operationID); notifyListeners("shareReceived", event); }
    void emitDraftReceived(String draftID) { JSObject event = new JSObject(); event.put("draftID", draftID); notifyListeners("shareDraftReceived", event); }
    void emitAssistantOpenRequested(String requestID) { JSObject event = new JSObject(); event.put("requestID", requestID); notifyListeners("assistantOpenRequested", event, true); }
}
