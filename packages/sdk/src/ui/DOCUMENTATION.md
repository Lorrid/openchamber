...
- `pull.ts`: `mountPullRequest`. Host PR window. `mode: 'create'` is title, description, head, base, draft. `mode: 'view'` is state, `head → base`, tabs Overview / Checks / Comments, and footer callbacks the guest passed. Merge and ready fire those callbacks. The kit does not call GitHub.
...
- `mountPullRequest` takes `mode`. Create needs `create.onSubmit`. View needs `pull`. Callbacks that are omitted stay off the footer. Title, body, checks, and comments are `textContent`.
