...
Sorted compaction Activity uses localized Compacting / Compaction complete labels with the same icon, geometry, typography, info-token shimmer, and duration formatter. While assistant Activity segments are pending, `TurnItem` renders the shared Activity presentation in status-only form from turn data (`shouldShowCompactionStatus`): settled completion always shows; active Compacting only when the turn is last and the session is authoritatively working. This form uses `role="status"` and presents a stable compact row. A visible segment activates the detail-bearing disclosure with the existing per-turn expansion state. Live mode keeps its current rendering.
...
- Each sorted turn uses one Activity container with the same full-width clickable header DOM, layout, typography, spacing, mobile behavior, and ARIA disclosure contract while collapsed and expanded. `activityRenderMode` supplies only an untouched turn's initial disclosure value; `summary` starts active turns expanded, every click reverses the current per-turn value, and touched values survive later setting changes. Before toggling, the header captures its viewport top; the next layout synchronously compensates `[data-scrollbar="chat"]` and performs one frame-level correction after virtualizer measurement, keeping the control under the pointer. Collapsed turns render only this header; expanded turns render the complete ordered detail timeline. Active headers apply `animate-text-shimmer` to localized Working text with `--oc-text-shimmer-base: var(--status-info)` and show the live duration. Normal and abnormal completed headers keep localized Processed text plus the authoritative duration across both disclosure states. An undefined completion disposition shows localized Processing details. Task inputs contribute their robustly parsed `subagent_type` to up to three unbordered square avatars separated by a small gap. Localized status text uses the authoritative total: active headers count running and pending Tasks, and completed headers count every participating Task.
