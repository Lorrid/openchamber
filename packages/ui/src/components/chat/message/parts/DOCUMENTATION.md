...
- Tool rows render synchronously. An expandable `ToolPart` that first mounts in the live streaming tail receives one 600ms opacity fade on its real `group/tool` row; the shared `FadeInDisabledProvider` keeps virtualized history remounts static, and reduced-motion CSS paints the row immediately. Shell completion can update tool output and row height without a wipe or tail-text animation.
