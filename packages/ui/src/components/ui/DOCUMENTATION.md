...
`MobileTabPageHeader` owns the safe-area-aware sticky large title for mobile root tabs. It binds to the nearest root tab or Settings scroller and writes scroll progress directly to `--oc-mobile-title-collapse`, which scales the title, shrinks top padding, and reveals the shared translucent navigation gradient. Expanded padding preserves the pre-collapse root rhythm (`safe-area + 1rem` page inset plus the legacy `pt-1.5` header offset); collapsed padding matches detail-navigation compact chrome. The scroll container zeros its own top padding so the header is the sole owner of that spacing.
