...
The sheet body is a flex column (`flex min-h-0 flex-1 flex-col overflow-hidden`) so children can own a bounded scroll region with `flex-1 min-h-0`. It also sets `data-page-scroll-lock="true"` so the global mobile-pointer rewrite that turns `.overflow-hidden` into `overflow-y: auto` does not promote the body itself into a competing scroll container. Searchable pickers (model, agent) must pin their search field outside the scroll region and put the list in `ScrollableOverlay` (or an equivalent `overflow-y-auto` with the `.overlay-scrollbar-container` class) so the dismiss gesture can recognize the list edge. Sessions already follows this pattern; model and agent pickers match it.
