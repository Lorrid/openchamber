...
`MobileTabPageHeader` owns the safe-area-aware sticky large title for mobile root tabs. It binds to the nearest root tab or Settings scroller and writes scroll progress directly to `--oc-mobile-title-collapse`, which scales the title and reveals the shared translucent navigation gradient. The header owns top safe-area spacing while its scroll container keeps the shared page inset.
