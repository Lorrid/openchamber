...
let volatileDeviceClientID: string | undefined;
const deviceClientID = () => { const key = 'openchamber:message-queue-import-client-id'; try { const storage = globalThis.localStorage; const existing = storage.getItem(key); if (existing) return existing; const created = createUuid(); storage.setItem(key, created); return created; } catch { return volatileDeviceClientID ??= createUuid(); } };
...
export const createMessageQueueShadowImporter = (overrides: Partial<Dependencies>): MessageQueueShadowImporter => {
  ...
  return { run: async (providedSignal) => {
    ...
    try {
      ...
      phase = 'create'; const kind = deps.kind ?? 'activation', clientID = deviceClientID(), expectedGeneration = deps.authorityGeneration?.() ?? capture.generation; const created = await deps.create!({ requestID: `shadow-create:${kind}:${expectedGeneration}:${clientID}:${snapshotHash}`, kind, clientID, snapshotHash, itemCount: prepared.length, protocol: 4, expectedGeneration, signal });
