...
`revertToMessage` and `unrevertSession` share a serial flight owned by `session-history-mutation-coordinator.ts`, keyed by `[transportIdentity, generation, directory, sessionId]`. Same-session revert and unrevert run in call order so concurrent HTTP cannot invert the server’s final marker; different sessions stay parallel. Each queued operation re-reads store state after the queue wait, re-validates runtime capture after every await, and must not publish marker/session/draft when the runtime is stale. A failed operation never blocks the next queued operation (tail releases in `finally`). Draft revision CAS rollback on remote failure is unchanged. `session-actions.ts` keeps domain operations only. `stageMessageEdit` accepts optional `{ directory, draftKey }` (defaults to primary `sessionDraftKey`) and returns an opaque `StageMessageEditHandle` whose `rollback()` CAS-restores the pre-stage draft or true absence (conflict keeps newer user edits); primary callers may ignore the handle. Primary composer edit commits abort any still-busy turn, dispatch the replacement first, and only then call `commitMessageEdit` with `preserveMessageId` set to the accepted replacement. A failed resend therefore keeps the aborted tail and the staged edit armed for retry; a successful resend deletes only the edited target and the old forward tail. `commitMessageEdit` accepts an optional directory override so Assistant continuous edit deletes against the correct child store, and an optional `preserveMessageId` so the just-accepted replacement (and anything after it) is never a delete candidate. Without `preserveMessageId` it still aborts a busy session for legacy callers. Its delete range comes from the authoritative snapshot `refetchSessionMessages` returns — forward only from the target (never a message older than it) and server-known ids only, so an in-flight optimistic replacement is not a delete candidate even when the server already echoed it back. Nothing is hidden up front: a row leaves the local store only after its own remote delete succeeds, so a failed abort / refetch / delete cannot leave the transcript out of sync with the server.
...
7. Edit staging restores the composer from the visible user-message snapshot captured at click time. Primary send aborts any still-busy turn, dispatches the replacement, and only after that acceptance refetches authoritative messages to determine the deletion range, deleting strictly forward from the target while preserving the replacement id. Each local row drops only as its remote delete lands. Submit paints the target as "editing" (`messageEditCommitting`) instead of hiding it, so the pending edit and the optimistic reply row never contend for the same transcript. A failed replacement send leaves the old tail and the staged edit intact. Leaving the session disarms `stagedMessageEdit`.
