...
The mobile Composer's flow slot reads `--oc-mobile-composer-shrink` (`0..1`) from `useChatAutoFollow`. Per scroll frame the handler writes exactly one CSS variable (no React state) and compensates the geometry feedback — retracted composer pixels grow the scroll viewport, so raw distance is corrected by the live shrink × stage delta before mapping the first 100px to progress. Queue / changes / todos keep their existing chrome and only clip on max-height (no opacity, no width change, no restyle). The expanded input keeps its original background, shadow, and blur at every stage; only the discrete `oc-mobile-composer-shrunk` flip narrows to 80%, overlays the transcript, and mixes the fill translucent with a backdrop blur (never interpolated per frame — color-mix/backdrop-filter repaint per geometry change is not acceptable on the scroll path). `prefers-reduced-transparency` keeps the opaque fill. Keyboard and input-pin overrides publish zero without introducing a React scroll-frame state path.
